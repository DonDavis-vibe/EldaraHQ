...
// Alternative zum eigenen Hochladen: referenziert die Datei direkt per Pfad statt
// sie wie beim Upload als Data-URL einzubetten - die Dateien dort sind schon fürs
// Web komprimiert, ein erneutes Verkleinern ist nicht nötig. Liste kommt aus
// npc-token-bilder.js (erzeugt von assets/npc-grafiken/erzeuge-manifest.py).
let nscGaleriePickerId = null;

function nscListeGaleriePicker(id) {
    if (typeof NPC_TOKEN_BILDER === 'undefined' || !NPC_TOKEN_BILDER.length) {
        alert('Keine Galerie-Grafiken gefunden (assets/npc-grafiken/npc-token-bilder.js fehlt oder ist leer).');
        return;
    }
    nscGaleriePickerId = id;
    const body = document.getElementById('nsc-galerie-body');
    if (body) {
        body.innerHTML = NPC_TOKEN_BILDER.map(g => `
            <div class="nsc-galerie-gruppe">
                <h4>${escapeHtml(g.gruppe)} <span class="x-count">${g.bilder.length}</span></h4>
                <div class="nsc-galerie-grid">
                    ${g.bilder.map(b => `<button type="button" class="nsc-galerie-item" data-galeriebild="${escapeHtml(b.pfad)}" title="${escapeHtml(b.name)}">
                        <img src="${escapeHtml(b.pfad)}" alt="${escapeHtml(b.name)}" loading="lazy">
                        <span>${escapeHtml(b.name)}</span>
                    </button>`).join('')}
                </div>
            </div>`).join('');
        body.querySelectorAll('[data-galeriebild]').forEach(btn => btn.addEventListener('click', () => nscListeBildSetzen(nscGaleriePickerId, btn.dataset.galeriebild)));
    }
    const overlay = document.getElementById('nsc-galerie-modal-overlay');
    if (overlay) overlay.classList.add('active');
}

function nscListeGalerieSchliessen() {
    nscGaleriePickerId = null;
    const overlay = document.getElementById('nsc-galerie-modal-overlay');
    if (overlay) overlay.classList.remove('active');
}

function nscListeBildSetzen(id, pfad) {
    const eintrag = nscListe.find(n => n.id === id);
    if (!eintrag) return;
    eintrag.bild = pfad;
    nscListeSichern();
...
// setFigurGroesse) - wird auf einen schon platzierten Token nachgezogen, genau
// wie das Bild (siehe karteNscGroessenAnwenden in karten.js).
function nscListeGroesseAendern(id, delta) {
    const eintrag = nscListe.find(n => n.id === id);
    if (!eintrag) return;
    const aktuell = Number(eintrag.groesse) || 1;
    eintrag.groesse = Math.max(0.5, Math.min(8, Math.round((aktuell + delta) * 2) / 2));
    nscListeSichern();
    renderNscListeGm();
...
function renderNscListeGm() {
    ...
    const zeilen = nscListeSortiert().map(n => {
        ...
        const kartenAktiv = typeof karteNsPlatzieren === 'function' && typeof eldaraAktiv === 'function' && eldaraAktiv();
        return `
        <div class="nsc-item">
            <div class="nsc-item-kopf">
                ...
                ${kartenAktiv ? `<span class="nsc-groesse" title="Kartentoken-Größe in Feldern">
                    <button class="x-mini" data-nscgroesse="${escapeHtml(n.id)}:-0.5" title="Kleiner">−</button>
                    <span class="nsc-groesse-val">${Number(n.groesse) || 1}</span>
                    <button class="x-mini" data-nscgroesse="${escapeHtml(n.id)}:0.5" title="Größer">+</button>
                </span>` : ''}
                ${kartenAktiv ? `<button class="x-mini" data-nsckarte="${escapeHtml(n.id)}" title="Auf Karte platzieren"><i class="fa-solid fa-map-location-dot"></i></button>` : ''}
    ...
    box.querySelectorAll('[data-nscgalerie]').forEach(b => b.addEventListener('click', () => nscListeGaleriePicker(b.dataset.nscgalerie)));
