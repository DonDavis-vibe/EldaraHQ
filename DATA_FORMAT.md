# EldaraHQ - JSON Export Format

This document describes the structure of the `.json` files exported by **EldaraHQ**, the character sheet tool for "How to be a Hero". 
...
A rule package bundles a group's house rules so the sheet can apply them: a fixed talent list, progressive talent costs, a talent tree with rank/skill points, per-Wesen effects and special roll tables. This fork only ever plays one package, `eldora-arrrrr` (`hausregeln/eldora-arrrrr.js`), which every browser loads automatically on startup - there is no UI to pick or switch packages, and nothing is synced between GM and players for it.

Only the character's *choices* within that package (`hausregeln` above, e.g. chosen branches/Wesen/learned skills) travel with the character's own JSON.
