...
function renderGmDashboard() {
    ...
    Object.keys(connectedPlayersData).forEach(peerId => {
        ...
        if (typeof hausregelnBudget === 'function' && hausregelnBudget() && pData.hausregeln && pData.hausregeln.paket === 'eldora-arrrrr') {
...
function joinMultiplayerSession(codeArg) {
    ...
    peer.on('open', () => {
        ...
        hostConnection.on('data', (payload) => {
            if (payload && payload.type === 'playSound') {
...
// (schiffsinventar.js) und Quest-Log (quests.js) liegen bislang jeder für
// sich in localStorage und persistieren dadurch schon automatisch - "an
// einem anderen Abend nahtlos weiterspielen" funktioniert also im selben
// Browser bereits ohne Zutun.
